...
[role="lead"]
Quando o sistema estiver em switchover, NetApp recomenda que você evite executar certas operações.

As seguintes operações não são recomendadas quando o sistema está em switchover e devem ser executadas somente sob orientação do suporte da NetApp:

* Criando ou excluindo agregados
* Criando ou excluindo volumes (incluindo volumes criptografados)
