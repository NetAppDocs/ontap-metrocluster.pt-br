...
Os LUNs e volumes ainda estão fisicamente hospedados nos nós antigos. Como os LUNs são relatados apenas nas interfaces dos nós antigos, o host mostra apenas os caminhos sobre os nós antigos. Para verificar isso, execute os comandos  `sanlun lun show -p` e  `multipath -ll -d` no host e examine as saídas dos comandos.
