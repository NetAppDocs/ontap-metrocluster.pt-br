...
Existem quatro RCFs, um para cada um dos quatro switches na configuração de fabric do MetroCluster. Você deve usar os RCFs corretos para o modelo de switch que está utilizando.
...
.Passos
. Faça o download dos arquivos RCF do Cisco FC a partir do  https://mysupport.netapp.com/site/products/all/details/metrocluster-rcf/downloads-tab["Página de download do MetroCluster RCF"].
. Copie os RCFs para os switches.
+
.. Copie o RCF para o primeiro switch:
+
`copy sftp://__root@FTP-server-IP-address/tftpboot/switch-specific-RCF__ bootflash: vrf management`

+
Neste exemplo, o  `NX3232_v1.80_Switch-A1.txt`RCF é copiado do servidor SFTP em  `10.10.99.99` para a memória flash de inicialização local. Você deve usar o endereço IP do seu servidor TFTP/SFTP e o nome do arquivo RCF que você precisa instalar.
...
IP_switch_A_1#
----
.. Repita a subetapa anterior para cada um dos outros três switches, certificando-se de copiar o RCF correspondente para o switch correspondente.


. Verifique em cada switch se o RCF está presente no diretório  `bootflash`de cada switch:
+
`dir bootflash:`
...
IP_switch_A_1#
----
. Copie o RCF correspondente da bootflash local para a configuração em execução em cada switch:
+
