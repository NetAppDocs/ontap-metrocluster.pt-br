...
.Informações relacionadas
Você precisa verificar se está usando as atribuições de porta especificadas ao conectar os switches FC. Consulte link:concept_port_assignments_for_fc_switches_when_using_ontap_9_1_and_later.html["Atribuições de portas para switches FC"]
...
image::../media/fcip_ports_dual_isl.gif[Diagrama mostrando portas FCIP com configuração ISL dupla em cada switch]
...
As etapas a seguir usam o zoneamento descrito na seção Zoneamento para um FibreBridge 7500N em uma configuração MetroCluster de quatro nós.
