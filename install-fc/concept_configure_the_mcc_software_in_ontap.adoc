...
image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[Fluxo de trabalho para configuração de cluster e nó de alto nível]
...
Exemplo utilizado neste procedimento: site_B
 a| 
...
Exemplo usado neste procedimento: controller_B_1
 a| 
 a| 
 a| 
 a| 
...
Exemplo usado neste procedimento: controller_B_2
 a| 
 a| 
 a| 
 a| 
...
[CAUTION]
====
* No Fabric OS 10.x e versões posteriores, Brocade suporta apenas SHA-512 para autenticação SNMP. ONTAP atualmente não suporta SHA-512, com suporte limitado a SHA-256.
* Se seus switches FC MetroCluster estiverem executando o Fabric OS 10.x ou posterior, você não poderá configurar SNMP nem adicionar esses switches ao monitoramento de integridade do ONTAP. Se você quiser fazer downgrade do firmware do switch, pode seguir as etapas em link:../maintain/task_upgrade_or_downgrad_the_firmware_on_a_brocade_fc_switch_mcc.html["Atualize ou faça o downgrade do firmware em um switch Brocade FC"].
