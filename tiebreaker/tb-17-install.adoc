...
summary: Execute uma nova instalação ou atualização para o tiebreaker 1,6 no sistema operacional Linux host para monitorar as configurações do MetroCluster. 
---
= Instale o MetroCluster Tiebreaker 1.7
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Instale ou atualize para o Tiebreaker 1.7 em seu host Linux para monitorar as configurações do MetroCluster .

.Sobre esta tarefa
* O sistema de storage deve estar executando o ONTAP 9.12,1 ou posterior.
* Você só pode atualizar para o Tiebreaker 1.7 a partir do Tiebreaker 1.6P1. Consultelink:tb-16-install.html["Instale o Tiebreaker 1.6 ou 1.6P1."] .
...
NOTE: Após instalar o Tiebreaker 1.7, você precisa link:concept_configuring_the_tiebreaker_software.html#import-certificates-tb17["importar certificados"] para ativar o monitoramento contínuo.

.Passos
. Baixe o software MetroCluster Tiebreaker 1.7.
+
...
. Se você estiver atualizando, verifique qual versão do Tiebreaker está em execução:
+
O exemplo a seguir mostra o Tiebreaker 1.6P1.
...
[role="tabbed-block"]
====
.Instale o Tiebreaker 1.7
--
Siga os passos abaixo para uma nova instalação do Tiebreaker 1.7.

.Passos
.. Execute o seguinte comando no `[root@mcctb ~] #` prompt para iniciar a instalação:
+
[source, cli]
----
...
--
.Atualize da versão 1.6P1 para a 1.7.
--
Siga os passos abaixo para atualizar o software Tiebreaker da versão 1.6P1 para a versão 1.7.

.Passos
.. Execute o seguinte comando no `[root@mcctb ~] #` prompt para atualizar o software:
+
[source, cli]
----
sh MetroClusterTiebreakerInstall-1.7
...
NOTE: Se a sua atualização falhar com um erro devido à falta de certificados, consultelink:../tiebreaker/concept_configuring_the_tiebreaker_software.html#import-certificates-tb17["Certificados de importação"] .  Após a importação dos seus certificados, você poderá tentar atualizar novamente.
...
.O que se segue?
Após instalar ou atualizar o Tiebreaker com sucesso, link:configure-tb-workflow.html["configurar o software MetroCluster Tiebreaker"] para adicionar as configurações do MetroCluster e importar os certificados necessários.
