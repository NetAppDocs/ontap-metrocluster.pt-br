...
[role="lead"]
Você pode precisar atualizar um arquivo de configuração (RCF) em um switch IP da Cisco. Por exemplo, uma atualização do ONTAP ou uma atualização do firmware do switch exigem um novo RCF.
...
[cols="3*"]
|===
| Se estiver a ligar o seguinte armazenamento... | Escolha o tipo de armazenamento Cisco 9336C-FX2... | Exemplo de banner/MOTD do RCF 
...
* O https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator["RcfFileGenerator"] deve ser capaz de criar o mesmo RCF que é aplicado, com a mesma versão e configuração (plataformas, VLANs).
* O RCF aplicado não foi fornecido pela NetApp para uma configuração especial.
* O RCF não foi alterado antes de ser aplicado.
* Os passos para restaurar as configurações de fábrica do switch foram seguidos antes de aplicar o RCF atual.
* Não foram feitas alterações na configuração do switch(port) após a aplicação do RCF.
+
Se você não atender a esses requisitos, não poderá usar os CleanUpFiles que são criados ao gerar os RCFs. No entanto, você pode aproveitar a função para criar CleanUpFiles genéricos — a limpeza usando esse método é derivada da saída de  `show running-config` e é considerada uma boa prática.
...
. Determine a versão atual do RCF e quais portas e VLANs são usadas: `IP_switch_A_1# show banner motd`
+
...
| 2 | 11, 12 | 259 | 259 
|===
. [[Criar-arquivos-RCF-e-CleanUpFiles-ou-criar-genérico-CleanUpFiles]] Crie os arquivos RCF e CleanUpFiles, ou crie genéricos CleanUpFiles para a configuração atual.
+
...
[role="tabbed-block"]
====
.Opção 1: criar os RCFs e CleanUpFiles
--
Use este procedimento se a configuração atender aos requisitos.

.Passos
.. Use o RcfFileGenerator 1.4a (ou posterior) para criar os RCFs com as informações que você obteve na Etapa 1. A nova versão do RcfFileGenerator cria um conjunto adicional de CleanUpFiles que você pode usar para reverter algumas configurações e preparar o switch para aplicar uma nova configuração de RCF.
.. Compare o banner motd com os RCFs que estão atualmente aplicados. Os tipos de plataforma, tipo de switch, porta e uso de VLAN devem ser os mesmos.
+

NOTE: Você deve usar a CleanUpFiles da mesma versão do RCF e para a mesma configuração exata. Usar qualquer CleanUpFile não funcionará e pode exigir uma reinicialização completa do switch.

+

NOTE: A versão do ONTAP para a qual o RCF foi criado não é relevante. Apenas a versão do RCF é importante.

+

NOTE: O RCF (mesmo que seja a mesma versão) pode listar menos ou mais plataformas. Certifique-se de que sua plataforma esteja listada.
...
--
====
. [[Criar os novos arquivos RCF para a nova configuração]] Crie os "novos" arquivos RCF para a nova configuração. Crie esses arquivos da mesma forma que você criou os arquivos na etapa anterior, exceto que você deve escolher a respectiva versão do ONTAP e do RCF.
+
Após concluir esta etapa, você deverá ter dois conjuntos de RCFs, cada conjunto consistindo de doze arquivos.

. Baixe os arquivos para o bootflash.
+
.. Baixe o CleanUpFiles que você criou em <<Create-RCF-files-and-CleanUpFiles-or-create-generic-CleanUpFiles,Crie os RCFs e CleanUpFiles, ou crie CleanUpFiles genéricos para a configuração atual>>
+

NOTE: Este CleanUpFile é para o RCF atual que está aplicado e *NÃO* para o novo RCF para o qual você deseja atualizar.

+
Exemplo de CleanUpFile para Switch-A1: `Cleanup_NX9336_v1.81_Switch-A1.txt`

.. Baixe os 'novos' RCFs que você criou em <<Create-the-new-RCF-files-for-the-new-configuration,Crie os "novos" RCFs para a nova configuração.>>
+
Exemplo de RCF para Switch-A1: `NX9336_v1.90_Switch-A1.txt`

.. Faça o download do CleanUpFiles que você criou <<Create-the-new-RCF-files-for-the-new-configuration,Crie os "novos" RCFs para a nova configuração.>>. Esta etapa é opcional — você pode usar o arquivo no futuro para atualizar a configuração do switch. Ele corresponde à configuração aplicada atualmente.
+
...
+
Se você constatar que algum dos itens acima está configurado, talvez não seja possível aplicar uma nova configuração RCF. No entanto, você pode reverter para a configuração anterior reiniciando o switch *sem* salvar a configuração em execução na configuração de inicialização. O switch será inicializado com a configuração anterior.



. Aplique o RCF e verifique se as portas estão online.
+
.. Aplique o RCF.
+
[listing]
...
----
. Modifique todas as LIFs do cluster para habilitar a reversão automática:
+
.. Reative a reversão automática:
+
...
cluster_A::*>
----
.. Reverter todas as LIFs do cluster que não estão atualmente em suas portas de origem para suas portas de origem:
+
`network interface revert -vserver Cluster -lif *`

+
Aguarde alguns minutos para que as LIFs retornem à sua porta inicial:

.. Confirme se todos os LIFs estão em suas portas iniciais:
+
[listing]
----
...
----
.. Verifique o estado dos MetroCluster nós:
+
