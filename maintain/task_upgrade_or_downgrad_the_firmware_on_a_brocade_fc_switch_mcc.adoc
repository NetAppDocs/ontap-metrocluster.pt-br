...
---
= Atualize ou faça o downgrade do firmware em um switch Brocade FC MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Antes de começar
Analise cuidadosamente as seguintes informações antes de utilizar este procedimento:

* A partir do Fabric OS 10.x, Brocade oferece suporte apenas ao SHA-512 para autenticação SNMP. ONTAP atualmente não oferece suporte ao SHA-512, sendo o suporte limitado ao SHA-256.
+
Se você atualizar o firmware do switch FC do MetroCluster para o Fabric OS 10.x, a visibilidade do monitoramento SNMP a partir do ONTAP será perdida. Isso aumenta o risco durante a solução de problemas e leva à perda de alertas proativos.

+
Se você atualizar ou fizer downgrade do Fabric OS 10.x, deverá executar as tarefas obrigatórias descritas em link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/10-0-x/Upgrading-and-Downgrading-Firmware/v24512876.html["Brocade Fabric OS Software Upgrade Guide, 10.0.x - Pré-requisitos para atualização ou downgrade"^].

+

CAUTION: Você deve considerar cuidadosamente essas limitações antes de prosseguir com a atualização ou o downgrade para ou a partir do Fabric OS 10.x ou posterior.

* A partir do Fabric OS 9.0.1, o SNMPv2 não é compatível em switches Brocade. Se você atualizar para o Fabric OS 9.0.1 ou posterior, você deve usar o SNMPv3 para monitoramento de integridade. Para mais informações, consulte link:../install-fc/concept_configure_the_mcc_software_in_ontap.html#snmpv3-mcc-fc["Configurando o SNMPv3 em uma configuração MetroCluster"].
* Se você estiver atualizando para o Fabric OS 9.2.x ou posterior, é necessário ter um certificado Brocade TruFOS instalado. Consulte link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/9-2-x/Obtaining-Firmware/download-prerequisites-new/brocade-trufos-certificates.html["Guia de atualização de software do Brocade Fabric os, 9,2.x"^] para obter mais informações.


.Sobre esta tarefa
Confirme se você verificou e executou as seguintes tarefas para sua configuração:

* Verifique se a nova versão do firmware é compatível com a sua solução. Veja o link:https://hwu.netapp.com/["Hardware Universe"^] para mais informações.
