...
NOTE: Se você estiver usando um switch Cisco 9336C-FX2 em uma configuração compartilhada conectando os shelves NS224, alterar as ISLs pode exigir um novo arquivo de configuração (RCF). Você não precisa de um novo RCF se as velocidades atuais e novas das ISLs forem 40 Gbps e 100 Gbps. Todas as outras alterações na velocidade da ISL exigem um novo RCF. Por exemplo, alterar a velocidade da ISL de 40 Gbps para 100 Gbps não exige um novo RCF, mas alterar a velocidade da ISL de 10 Gbps para 40 Gbps exige um novo RCF.
...
NOTE: Se você estiver usando os switches Cisco 9336C-FX2 em uma configuração compartilhada conectando os shelves NS224 e precisar atualizar o RCF e aplicar a nova configuração para as novas portas ISL, siga as etapas para link:task_upgrade_rcf_files_on_mcc_ip_switches.html["Atualize os RCFs nos switches IP do MetroCluster."]
