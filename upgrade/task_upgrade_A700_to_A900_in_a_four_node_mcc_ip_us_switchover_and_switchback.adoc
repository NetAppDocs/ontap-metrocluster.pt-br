...
. Se os nós MetroCluster estiverem configurados com um gerenciador de chaves externo para o SVM de administração (cluster), copie as informações do servidor de chaves para um local seguro. Se o Onboard Key Manager estiver configurado, copie as informações de backup e a senha para um local seguro.
+
[role="tabbed-block"]
====
.Gerenciador de Chaves Integrado
--
.. Execute o seguinte comando para exibir o backup do Onboard Key Manager:
+
`security key-manager onboard show-backup`

+
Você também precisará da senha mais recente do Onboard Key Manager posteriormente no procedimento de upgrade.
...
--
Reúna as informações de gerenciamento de chaves externas que você precisará para restaurar a configuração do gerenciador de chaves posteriormente no procedimento.

.. Exibir o nome da SVM de administração (cluster).
+
`vserver show -type admin`

+
Neste exemplo, o nome da SVM administrativa é cluster_B:
...
cluster_B
            admin   -          -          -           -          -
----
.. Obtenha o endereço e o número da porta de um servidor de chave primária associado ao gerenciador de chaves externo configurado no SVM de administração (cluster):
+
[NOTE]
====
*** O SVM de administração pode ter até quatro servidores de chave primária, mas você precisará apenas do número da porta e do endereço de um dos servidores de chave primária para restaurar a configuração de gerenciamento de chaves posteriormente no procedimento.
*** Se houver um nome de usuário e senha associados ao servidor de chave primária escolhido, você também precisará dessas informações para o processo de recuperação.
...
----
.. Se você estiver executando ONTAP 9.16.1 ou posterior, obtenha o UUID do keystore para a SVM de administração.
+
...
cluster_B::*
----
.. Guarde as informações que você coletou em um local seguro para uso posterior no procedimento. Você deverá ter coletado as seguintes informações:
+
*** O número da porta e o endereço de um dos servidores de chave primária para a SVM de administração
*** O nome de usuário e a senha se houver associados ao servidor de chave primária escolhido
*** O UUID do keystore para a SVM de administração (necessário no ONTAP 9.16.1 ou posterior)
...
Desempate
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations["Remoção das configurações do MetroCluster"] no conteúdo _Instalação e Configuração do Desempate do MetroCluster_
...
.Antes de começar
* Verifique se você consegue acessar um servidor HTTP com o sistema.
...
+
.. Digitar `n` Para ignorar a recuperação do backup quando vir a seguinte mensagem:
+
....
Do you want to restore the backup configuration now? {y|n} n
....
.. Digitar `y` Reinicie o computador quando vir a seguinte mensagem:
+
....
...
.Passos
. Prepare os switches IP para a aplicação dos novos arquivos de configuração de referência (RCFs).
+
Siga as etapas na seção para o fornecedor do switch:
...
. Se a SVM administrativa do MetroCluster sistema estiver configurada com um gerenciador de chaves, selecione a opção correta do menu de inicialização para a sua configuração de gerenciamento de chaves.
+
|===
...
Siga as instruções para fornecer as seguintes informações para recuperar e restaurar a configuração do gerenciador de chaves:

** Backup e senha do Onboard Key Manager
...
** O número da porta e o endereço de um dos servidores de chave primária para a SVM de administração
** O nome de usuário e a senha se um estiver associado ao servidor de chave primária escolhido
** O UUID do keystore para a SVM de administração (necessário no ONTAP 9.16.1 ou posterior)


*Nota:* Embora a SVM administrativa possa ter até quatro servidores de chave primária, apenas as informações de um servidor de chave primária podem ser especificadas durante o processo de recuperação.
...
. Se a criptografia do volume raiz estiver sendo usada, selecione novamente a opção do menu de inicialização para a configuração de gerenciamento de chaves.
+
|===
...
NOTE: Ao executar a opção do menu de inicialização  `6`, os valores de bootarg do MetroCluster podem precisar ser redefinidos. Consulte link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#set-the-metrocluster-ip-bootarg-variables["Defina as variáveis MetroCluster IP bootarg"].
...
Desempate
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Adição de configurações do MetroCluster"] na seção _Instalação e Configuração do Desempate do MetroCluster_.
