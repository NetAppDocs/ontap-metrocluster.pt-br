...
* A tabela mostra todos os modelos de plataforma que usam *portas compartilhadas de MetroCluster/HA* e *portas compartilhadas de cluster/HA*. Nem todas as combinações são suportadas por este procedimento. Consulte a <<expand-ip-supported-platforms,combinações de plataforma suportadas>> para verificar se sua combinação de expansão é suportada.
...
As tabelas a seguir mostram as combinações de plataforma suportadas para configurações IP de oito nós do MetroCluster.
...
== Remova a configuração dos aplicativos de monitoramento
...
== Prepare os novos módulos do controlador
...
. No modo de manutenção, emita o comando halt para sair do modo de manutenção:
+
`halt`
...
. Inicialize o sistema e acesse a configuração do cluster:
+
`boot_ontap`
...
== Configurar LIFs entre clusters para o emparelhamento de clusters
...
+
Para obter a sintaxe completa do comando, consulte o link:https://docs.netapp.com/us-en/ontap-cli["Referência de comandos ONTAP"^].
...
+
O exemplo a seguir atribui as portas "e0e" e "e0f" ao grupo de failover "intercluster01" na SVM "cluster01":
...
+
O exemplo a seguir mostra que as LIFs intercluster "cluster01_icl01" e "cluster01_icl02" na porta "e0e" farão failover para a porta "e0f".
...
== Crie as interfaces MetroCluster e espelhe os agregados raiz

Você deve criar as interfaces MetroCluster nos novos nós IP MetroCluster e espelhar os agregados raiz.
...
.Passos
. Em cada site, adicione os novos LIFs intercluster ao relacionamento entre pares.
+
A partir do cluster_A, execute o seguinte comando:
...
+
A partir do cluster_B, execute o seguinte comando:
...
NOTE: O resultado do  `metrocluster configuration-settings connection show` comando pode variar dependendo do modelo da sua plataforma. Por exemplo, em algumas plataformas, as linhas referentes ao HA Partner não são exibidas na saída.
...
== Finalize a adição dos novos nós
