...
NOTE: Quando você executa a opção do menu de inicialização  `6`, os valores de bootarg do MetroCluster podem precisar ser redefinidos. Consulte link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Defina as variáveis MetroCluster IP bootarg"].
