...
. Se a criptografia do volume raiz estiver sendo usada, dependendo da versão do ONTAP que você estiver utilizando, selecione a opção do menu de inicialização ou execute novamente o comando do menu de inicialização para a configuração de gerenciamento de chaves.
+
[role="tabbed-block"]
====
.ONTAP 9 F.8 e mais tarde
