...
[role="lead"]
Você deve aplicar os arquivos de configuração de referência (RCFs) para reconfigurar seu zoneamento para acomodar os novos nós.

Se não for possível usar os RCFs para configurar os switches, você deve configurá-los manualmente. Consulte:
...
.Passos
. Localize os RCFs para sua configuração.
+
Você deve usar os RCFs para uma configuração de oito nós que corresponda ao modelo do seu switch.

. Aplique os RCFs, seguindo as instruções na página de download, ajustando as configurações do ISL conforme necessário.
...
+
O uso da porta do switch FC deve corresponder ao uso de oito nós do MetroCluster descrito em link:../install-fc/index.html["Instalação e configuração do MetroCluster conectado à malha"] para que os RCFs possam ser utilizados.
