...
Verifique se as LIFs estão hospedadas nos nós e portas apropriados antes de realizar o switchback.
...
CAUTION: Você deve verificar se a localização das LIFs de dados está correta nos novos nós antes de realizar o switchback. Quando você realizar o switchback da configuração, o ONTAP tentará retomar o tráfego na porta inicial usada pelas LIFs. Uma falha de E/S pode ocorrer quando a conexão da porta inicial com a porta do switch e a VLAN estiver incorreta. Se você encontrar qualquer problema ao posicionar as LIFs nos nós ou portas corretos, não prossiga com o switchback e entre em contato com o suporte técnico da NetApp.
...
|===
. Realize as seguintes verificações para confirmar se o switchover entre os dois clusters está operacional:
+
.. A partir do cluster de DR, verifique se um switchover do cluster em funcionamento para o cluster de DR é possível:
+
`metrocluster switchover -simulate`

.. A partir do cluster sobrevivente, verifique se um switchover do cluster de recuperação de desastres para o cluster sobrevivente é possível:
+
`metrocluster switchover -simulate`
...
NOTE: Em um ambiente FC do MetroCluster, "conexões" devem aparecer como "não aplicável" na saída do comando.
