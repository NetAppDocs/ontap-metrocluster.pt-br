...
NOTE: Você só pode configurar um ONTAP Mediator que usa HTTPS por meio da CLI. System Manager não é compatível. Para obter mais informações, consulte link:concept_mediator_requirements.html#select-a-communication-protocol-for-ontap-mediator["Selecione um protocolo de comunicação para ONTAP Mediator"].
