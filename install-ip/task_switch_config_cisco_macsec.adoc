...
summary: Caso necessário, configure a criptografia MACsec apenas nas portas WAN ISL que interligam os sites. Se optar por usar a criptografia MACsec, configure-a após aplicar o arquivo RCF correto. 
---
= Configurar criptografia MACsec em switches Cisco 9336C em um site IP MetroCluster (opcional)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Caso necessário, você pode configurar criptografia MACsec nas portas WAN ISL que interligam os sites. Se optar por usar criptografia MACsec, você deve configurá-la após aplicar o arquivo RCF correto.

[NOTE]
====
* A criptografia MACsec só pode ser aplicada às portas WAN ISL que interligam os sites.
* Configurar a criptografia MACsec é opcional ao configurar switches Cisco 9336C.
