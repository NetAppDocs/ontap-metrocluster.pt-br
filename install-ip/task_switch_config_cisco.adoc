...
---
= Configurar switches IP da Cisco para interconexão de cluster e conectividade IP MetroCluster de backend
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Se você estiver usando essas etapas como parte de outro procedimento, como a substituição ou atualização de um switch, verifique se é necessário mover as LIFs do cluster para o switch parceiro para permitir operações de cluster sem interrupções. link:task_install_and_cable_the_mcc_components.html["Saiba como configurar os switches IP do MetroCluster"].
...
.Sobre esta tarefa
Essa tarefa requer um software de transferência de arquivos, como FTP, TFTP, SFTP, HTTP ou SCP, para copiar os arquivos para os switches.

* Se você estiver usando essas etapas como parte de outro procedimento, como a substituição ou atualização de um switch, verifique se é necessário mover as LIFs do cluster para o switch parceiro para permitir operações de cluster sem interrupções. link:task_install_and_cable_the_mcc_components.html["Saiba como configurar os switches IP do MetroCluster"].
* Você deve usar a versão de software de switch compatível. Consulte o  https://hwu.netapp.com["NetApp Hardware Universe"].
...
. Repita os passos anteriores nos três switches IP restantes na configuração IP do MetroCluster.
+

NOTE: Você deve atualizar os switches na seguinte ordem: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. Se o seu procedimento exigir que você mova as LIFs de cluster, será necessário repetir as etapas para mover as LIFs de cluster na segunda malha (Switch_A_2, Switch_B_2) após configurar os switches na primeira malha (Switch_A_1, Switch_B_1). Consulte link:task_install_and_cable_the_mcc_components.html["Saiba como configurar os switches IP do MetroCluster"].

. Verifique se a reversão automática está habilitada para as LIFs do cluster:
+
`network interface show -vserver Cluster -fields auto-revert`

+

CAUTION: Você deve garantir que a reversão automática esteja habilitada para as LIFs do cluster.

. Se a reversão automática não estiver ativada, modifique todas as LIFs do cluster para ativar a reversão automática:
+

NOTE: Se você estiver executando essas etapas como parte de outro procedimento, como a substituição ou atualização de um switch, você pode ter desativado a reversão automática como parte das etapas para link:switch-move-cluster-lifs.html["Mova os LIFs do cluster"].
