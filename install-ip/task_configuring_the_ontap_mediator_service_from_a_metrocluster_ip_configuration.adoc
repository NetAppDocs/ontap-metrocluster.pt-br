...
* Essa tarefa pode ser executada na interface ONTAP de qualquer nó na configuração IP MetroCluster.
...
NOTE: Você será solicitado a inserir o nome de usuário e a senha da conta de admin do Mediator.
...
. Opcionalmente, configurar MetroCluster switchover forçado automático.
+
Você só pode usar o seguinte comando em um nível de privilégio avançado.

+

IMPORTANT: Antes de usar este comando, revise o link:concept-ontap-mediator-supports-automatic-unplanned-switchover.html#mauso-9-12-1["Riscos e limitações do uso do switchover forçado automático do MetroCluster"].
