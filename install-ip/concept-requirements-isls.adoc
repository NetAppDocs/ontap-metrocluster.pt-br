...
+
** A partir do ONTAP 9.18.1, as plataformas com velocidade de porta IP de backend MetroCluster de 100G exigem uma velocidade mínima de link ISL de 100Gbps. Utilizar uma velocidade ISL diferente requer uma Solicitação de Variação de Recurso (FPVR). Para registrar uma solicitação FPVR, entre em contato com sua equipe de vendas da NetApp .
