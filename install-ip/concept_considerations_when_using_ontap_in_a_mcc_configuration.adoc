...
---
= Requisitos e considerações para operações ONTAP com configurações de IP do MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image::../media/svm_dr_normal_behavior.gif[Recuperação de desastres SVM durante operação em estado estável]
...
+
image::../media/svm_dr_image_2.gif[Atualizações do SVM após a transição]
...
image::../media/svm_dr_resynchronization.gif[Ressincronização de recuperação de desastres SVM]
