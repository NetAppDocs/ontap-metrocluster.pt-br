...
Novas combinações de atualização suportadas para MetroCluster atualizações de controladores IP usando  `system controller replace` comandos
 a| 
Suporte para atualizações de sistema de AFF A70 para AFF A90 e de FAS70 para FAS90 usando  `system controller replace` comandos em uma configuração IP MetroCluster.
